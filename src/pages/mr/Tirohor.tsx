...
const Tirohor = () => {
  ...
  const [yIni, setYIni] = useState<number>(10);
  const ang = 0; // 0 grados en radianes
  ...
  function trayectoria() {
    ...
    velYExp = [];
    velYTeo = [];
    const tTray = Math.sqrt(2 * Math.abs(G) * yIni) / Math.abs(G); // Tiempo total de vuelo
    console.log(tTray);
    for (let i = 0; i < N; i++) {
      const t = (tTray * i) / (N - 1);
  ...
  return (
    <>
      <Layout>
        <h1 style={{ textAlign: "center" }}>Tiro Horizontal</h1>
        ...
        >
          ...
          >
            <Button
              onClick={trayectoria}
              disabled={
                masa <= 0 || N <= 0 || N > 500 || velI <= 0 || yIni <= 0
              }
            >
